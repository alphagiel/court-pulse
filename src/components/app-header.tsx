...
interface AppHeaderProps {
  ...
  onBack?: () => void;
  /** Show "Ladder" pill link (home page only) */
  showLadderLink?: boolean;
}
  ...
  return (
    <div className="flex items-center justify-between gap-2">
      ...
      <div className="w-[80px] flex items-center">
        ...
        {showLadderLink && (
          <button
            ...
            className="text-[13px] text-green-700 font-medium border border-green-200 bg-green-50 rounded-full px-3 py-1 hover:bg-green-100 transition-colors"
          >
            Ladder
          </button>
        )}
      ...
      <div className="flex-1 text-center min-w-0">
        <h1 className="text-[20px] sm:text-[22px] font-bold tracking-[0.5px] truncate">{title}</h1>
        {subtitle && (
      ...
      {/* Right: Greeting + hamburger */}
      <div className="w-[80px] flex items-center justify-end gap-1.5 relative" ref={menuRef}>
        <span className="text-[12px] text-muted-foreground font-medium truncate max-w-[55px] hidden sm:inline">
          Hi {username}
        </span>
        <button
        ...
        {menuOpen && (
          <div className="absolute right-0 top-10 z-50 bg-card border border-border rounded-lg shadow-lg py-1 min-w-[140px]">
            <div className="px-3 py-2 border-b border-border sm:hidden">
              <p className="text-[13px] font-medium truncate">Hi {username}</p>
            </div>
            <button
            ...
            <button
              onClick={() => { setMenuOpen(false); router.push("/ladder"); }}
              className="w-full text-left px-3 py-2 text-[13px] text-muted-foreground hover:text-foreground hover:bg-muted transition-colors"
            >
              Ladder
            </button>
